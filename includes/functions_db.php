 ...
 *
 * @version $Id: functions_db.php,v 1.1 2005/12/29 19:36:21 lsces Exp $
 * @package PhpGedView
 * @subpackage DB
 */
...
function setup_database() {
	...
	if (($tables_exist)&&(!$has_rin || !$has_first_letter)) {
		$sql = "DROP table if exists ".$TBLPREFIX."individuals, ".$TBLPREFIX."families, ".$TBLPREFIX."sources, ".$TBLPREFIX."other, ".$TBLPREFIX."places, ".$TBLPREFIX."media, ".$TBLPREFIX."media_mapping";
		$res = dbquery($sql);
	...
	if (!$tables_exist) {
		$sql = "CREATE TABLE ".$TBLPREFIX."individuals (i_id VARCHAR(255), i_file INT, i_rin VARCHAR(255), i_name VARCHAR(255), i_isdead INT DEFAULT 1, i_gedcom BLOB, i_letter VARCHAR(5), i_surname VARCHAR(100))";
		$res = dbquery($sql);
		...
		$sql = "CREATE TABLE ".$TBLPREFIX."families (f_id VARCHAR(255), f_file INT, f_husb VARCHAR(255), f_wife VARCHAR(255), f_chil BLOB, f_gedcom BLOB)";
		$res = dbquery($sql);
		...
		$sql = "CREATE TABLE ".$TBLPREFIX."sources (s_id VARCHAR(255), s_file INT, s_name VARCHAR(255), s_gedcom BLOB)";
		$res = dbquery($sql);
		...
		$sql = "CREATE TABLE ".$TBLPREFIX."other (o_id VARCHAR(255), o_file INT, o_type VARCHAR(20), o_gedcom BLOB)";
		$res = dbquery($sql);
		...
		$sql = "CREATE TABLE ".$TBLPREFIX."media (m_id INT NOT NULL, m_media VARCHAR(15), m_ext varchar(6), m_titl varchar(255), m_file varchar(255), m_gedfile BIGINT, m_gedrec BLOB, PRIMARY KEY (m_id))";
		$res = dbquery($sql);
		...
		$sql = "CREATE TABLE ".$TBLPREFIX."media_mapping (mm_id int NOT NULL, mm_media varchar(15) NOT NULL default '', mm_gid varchar(15) NOT NULL default '', mm_order int NOT NULL default '0', mm_gedfile int default NULL, mm_gedrec BLOB, PRIMARY KEY (mm_id))"; 
		$res = dbquery($sql);
	...
	if (!$has_dates) {
		$sql = "CREATE TABLE ".$TBLPREFIX."dates (d_day INT, d_month VARCHAR(5), d_mon INT, d_year INT, d_datestamp INT, d_fact VARCHAR(10), d_gid VARCHAR(255), d_file INT, d_type VARCHAR(13))";
		$res = dbquery($sql);
	...
	if (!$has_media) {
		$sql = "CREATE TABLE ".$TBLPREFIX."media (m_id int NOT NULL, m_media varchar(15) default '' NOT NULL, m_ext varchar(6) default '' NOT NULL, m_titl varchar(255) default NULL, m_file varchar(255) default '' NOT NULL, m_gedfile BIGINT default NULL, m_gedrec BLOB, PRIMARY KEY (m_id))";
		$res = dbquery($sql);
	...
	if (!$has_media_mapping) {
		$sql = "CREATE TABLE ".$TBLPREFIX."media_mapping (mm_id int NOT NULL, mm_media varchar(15) default '' NOT NULL, mm_gid varchar(15) default '' NOT NULL , mm_order INT default '0' NOT NULL, mm_gedfile BIGINT default NULL, mm_gedrec BLOB, PRIMARY KEY (mm_id))"; 
		$res = dbquery($sql);
